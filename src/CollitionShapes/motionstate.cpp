...
Bullet::MotionState::MotionState(QObject *parent) : QObject(parent),
    btDefaultMotionState()
{

}

Bullet::MotionState::MotionState(const btTransform &startTrans, const btTransform &centerOfMassOffset,QObject* parent)
    :QObject(parent),btDefaultMotionState(startTrans,centerOfMassOffset)
{

}
Bullet::MotionState::MotionState(const QMatrix4x4 startTrans, const QMatrix4x4 centerOfMassOffset,QObject* parent)
    :QObject(parent)
{
...
void Bullet::MotionState::setWorldTransform(const btTransform &centerOfMassWorldTrans){
    btDefaultMotionState::setWorldTransform(centerOfMassWorldTrans);
    btScalar* m=new btScalar[16];
    centerOfMassWorldTrans.getOpenGLMatrix(m);
    QMatrix4x4 worldTranform(m[0],m[4],m[8],m[12],
                            m[1],m[5],m[9],m[13],
                            m[2],m[6],m[10],m[14],
                            m[3],m[7],m[11],m[15]);
    emit motionStateChanged(worldTranform);
}
void Bullet::MotionState::getWorldTransform(btTransform &centerOfMassWorldTrans) const{
    return btDefaultMotionState::getWorldTransform(centerOfMassWorldTrans);
}


void Bullet::MotionState::setWorldTransform(QMatrix4x4 m){
...
QMatrix4x4 Bullet::MotionState::getWorldTransformAsQMatrix4x4(){
    btTransform matrix;
